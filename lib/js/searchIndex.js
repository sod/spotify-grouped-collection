exports.build = (function() {
  ...
  function filterRenderer(items, value) {
    ...
    for(; index < items.length; index += 1) {
      className = find.test(items[index].searchIndex) ? visible : hidden;
      if(items[index].element.className !== className) {
        items[index].element.className = className;
      }
    }
  }
  ...
  function checkForEmptyPlaylistContainer(container, has, baseClass) {
    ...
    for(; index < elements.length; index += 1) {
      elements[index].className = baseClass + ' ' + ( elements[index].querySelector(has) ? visible : hidden );
    }
  }
